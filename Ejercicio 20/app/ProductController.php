...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    switch ($_POST["action"]) {
        ...
        case "edit":
            ...
            $product = new ProductController();
            $product->edit($id, $name, $slug, $description, $features, $brand);
            break;
        case "delete":
            if(!isset($_SESSION['data']) && $_SESSION['token'] != $_POST['token']) {
...
class ProductController
{
    ...
    static function create($name, $slug, $description, $features, $brand, $image)
    {
        ...
        var_dump($code);
        if ($code == 4) {
            header("Location:" . BASE_PATH . "products/details/" . $slug);
        } else {
            header("Location:" . BASE_PATH . "home");
        }
    }

    static function edit($id, $name, $slug, $description, $features, $brand)
    {
        $curl = curl_init();
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => 'name=' . $name . '&slug=' . $slug . '&description=' . $description . '&features=' . $features . '&brand_id=' . $brand . '&id=' . $id . '&categories%5B0%5D=3&categories%5B1%5D=4&tags%5B0%5D=3&tags%5B1%5D=4',
            CURLOPT_HTTPHEADER => array(
